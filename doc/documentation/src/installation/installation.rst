...
Building 4C for the CUDA backend currently necessitates the use of a compiler wrapper, `utilities/clangcuda++`, as the C++ compiler and clang as the C compiler. The reason for this is that Nvidia's NVCC (or the `kokkos_launch_compiler` which is based on NVCC) has issues compiling some modern C++ code in 4C, and using clang resolves these issues. To build 4C for the CUDA backend, you must first enable the `FOUR_C_CLANGCUDA` compile option. For an MPI build of 4C (`-DCMAKE_CXX_COMPILER=</path/to/mpicxx>`; `-DCMAKE_C_COMPILER=</path/to/mpicc>`), you should then set the following environment variables: `export OMPI_CXX=</path/to/4C>/utilities/clangcuda++` and `export OMPI_CC=</path/to/clang>`. The compiler wrapper will detect and use the same architecture as the Kokkos installation, but, in case of errors or cross-compilation, you can explicitly override this by setting the `CLANGCUDA_ARCH` environment variable. Similarly, the default clang++ and CUDA paths can be overridden with the environment variables `CLANGCUDA_CLANG_PATH` and `CLANGCUDA_CUDA_PATH`, respectively.
