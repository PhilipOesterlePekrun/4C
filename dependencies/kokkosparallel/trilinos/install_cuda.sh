...
$CMAKE_COMMAND \
  ...
  \
  -D Trilinos_ENABLE_CUDA=TRUE \
  -D TPL_ENABLE_CUDA=TRUE \
  -D Kokkos_ENABLE_CUDA=TRUE \
  -D Kokkos_ARCH_HOPPER90=TRUE \
  -D Kokkos_ENABLE_CUDA_CONSTEXPR=TRUE \
  -D Tpetra_INST_CUDA=FALSE \
  -D KokkosKernels_ENABLE_TPL_CUSOLVER=TRUE \
  \
  ../Trilinos
